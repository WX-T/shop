		...
		foreach ($results as $result) {
		    ...
		    foreach ($products as $product) {
		        ...
		        $productList[] = array(
		            ...
		            'party_price'      => $product['party_price'],
...
// 	    $party_assbillno = $this->request->post['assbillno'];
	    $rs = $this->model_sale_order->confirmBuy($o_product_id , $party_order_no , $party_price);
	    if($rs){
	...
	public function shipment(){
	    ...
	    $assbillno = $this->request->post['assbillno'];
	    $ship_price = $this->request->post['ship_price'];
	    $party_assbillno = $this->request->post['party_assbillno'];
	    $this->model_sale_order->shipment($order_id , $shipping_agents , $assbillno , $ship_price,$party_assbillno);
	    $json = json_encode(array('code'=>'1'));
	    $this->response->addHeader('Content-Type: application/json');
	    $this->response->setOutput($json);
	}
	...
	public function doexport(){
	    ...
	    $exportdata = $this->model_sale_order->getexportdata($start_time,$end_time);
	    $count = count($exportdata);
	    //设置表头名称
	    $objPHPExcel->getActiveSheet()->setTitle('订单列表');
	    //表格标题栏
	    $objPHPExcel->getActiveSheet()->setCellValue('A1', '用户名');
	    $objPHPExcel->getActiveSheet()->setCellValue('B1', '联系电话');
	    ...
	    $objPHPExcel->getActiveSheet()->setCellValue('F1', '商品单价');
	    $objPHPExcel->getActiveSheet()->setCellValue('G1', '订单号');
	    $objPHPExcel->getActiveSheet()->setCellValue('H1', '订单总价');
	    $objPHPExcel->getActiveSheet()->setCellValue('I1', '运费');
	    $objPHPExcel->getActiveSheet()->setCellValue('J1', '第三方链接');
	    $objPHPExcel->getActiveSheet()->setCellValue('K1', '物流单号');
	    $objPHPExcel->getActiveSheet()->setCellValue('L1', 'amazon订单号');
	    $objPHPExcel->getActiveSheet()->setCellValue('M1', '订单状态');
	    $objPHPExcel->getActiveSheet()->setCellValue('N1', '下单日期');
	    $objPHPExcel->getActiveSheet()->setCellValue('O1', '修改日期');
	    $objPHPExcel->getActiveSheet()->setCellValue('P1', '是否走跨境电商');
	   //设置表格宽度
	   ...
	   $objPHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth(20);
        ...
        // 循环输出数据到excel
        for ($i = 2; $i <= $count + 1; $i ++) {
            $objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $exportdata[$i - 2]['fullname']);
            $objPHPExcel->getActiveSheet()->setCellValueExplicit('B' . $i,$exportdata[$i - 2]['telephone'],PHPExcel_Cell_DataType::TYPE_STRING);
            $objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $exportdata[$i - 2]['name']);
            $objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $exportdata[$i - 2]['model']);
            $objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $exportdata[$i - 2]['quantity']);
            $objPHPExcel->getActiveSheet()->setCellValue('F' . $i, $exportdata[$i - 2]['price']);
            $objPHPExcel->getActiveSheet()->setCellValue('G' . $i, $exportdata[$i - 2]['order_id']);
            $objPHPExcel->getActiveSheet()->setCellValue('H' . $i, $exportdata[$i - 2]['total']);
            $objPHPExcel->getActiveSheet()->setCellValue('I' . $i, $exportdata[$i - 2]['tariff_price']);
            $objPHPExcel->getActiveSheet()->setCellValue('J' . $i, $exportdata[$i - 2]['out_url']);
            $objPHPExcel->getActiveSheet()->setCellValue('K' . $i, $exportdata[$i - 2]['party_assbillno']);
            $objPHPExcel->getActiveSheet()->setCellValue('L' . $i, $exportdata[$i - 2]['party_order_no']);
            $objPHPExcel->getActiveSheet()->setCellValue('M' . $i, $exportdata[$i - 2]['orderstaus']);
            $objPHPExcel->getActiveSheet()->setCellValue('N' . $i, $exportdata[$i - 2]['date_added']);
            $objPHPExcel->getActiveSheet()->setCellValue('O' . $i, $exportdata[$i - 2]['date_modified']);
            if($exportdata[$i - 2]['declartype'] == '1'){
                $objPHPExcel->getActiveSheet()->setCellValue('P' . $i, '是');
            }elseif($exportdata[$i - 2]['declartype'] == '5'){
                $objPHPExcel->getActiveSheet()->setCellValue('P' . $i, '否');
            }else{
                $objPHPExcel->getActiveSheet()->setCellValue('P' . $i, '未确定');
            }
        }
