...
class ControllerCatalogManufacturer extends Controller {
	...
	protected function getForm() {
		...
		if(isset($this->error['detailed'])){
			$data['error_detailed'] = '输入描述';
		}else{
			$data['error_detailed'] = '';
		}
		...
		if (isset($this->request->post['detailed'])) {
			$data['detailed'] = $this->request->post['detailed'];
		} elseif (!empty($manufacturer_info)) {
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['detailed']) < 2) || (utf8_strlen($this->request->post['detailed']) > 300)) {
			$this->error['detailed'] = '描述不能少于2或大于200';
		}
