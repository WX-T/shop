...
class ModelAccountReturn extends Model {
	...
	public function checkGoodsIsReturn($order_id , $product_id){
	    $query = $this->db->query("SELECT r.product_id,rh.return_status_id FROM `" . DB_PREFIX . "return` r LEFT JOIN sl_return_history rh ON r.return_id=rh.return_id WHERE r.order_id = '" . $order_id . "' AND r.product_id='".$product_id."' ORDER BY return_history_id DESC");
	    return $query->row;
	}
}