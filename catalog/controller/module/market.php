    ...
    public function index($setting) {
        ...
        if (!empty($setting['images'])) {
            foreach ($setting['images'] as $key=>$image){
                $data['list'][$key]['image'] = $this->model_tool_image->resize($image['image']);
                $data['list'][$key]['link'] = $image['url'];
                foreach ($image['product'] as $product_id) {
        ...
        if ($data['list']) {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/market.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/module/market.tpl', $data);
            } else {
