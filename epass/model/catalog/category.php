...
class ModelCatalogCategory extends Model {
	...
	public function editCategory($category_id, $data) {
        ...
        $data['recommend'] = serialize($data['images']);
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "',recommend='".$data['recommend']."', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
