...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_account_customer->deleteLoginAttempts($this->request->post['fullname']);
			
			$this->customer->login($this->request->post['email'], $this->request->post['password']);
			$this->customer->login($this->request->post['fullname'], $this->request->post['password']);
			...
			if (isset($this->request->post['redirect']) && (strpos($this->request->post['redirect'], HTTP_SERVER) === 0 || strpos($this->request->post['redirect'], HTTPS_SERVER) === 0 )) {
			    ...
			    $this->response->redirect($this->url->link('common/waphome'));
			}
		}
	...
	public function validate() {
		...
		if(!preg_match('/^[0-9a-zA-Z_\x{4e00}-\x{9fa5}]+$/u',$this->request->post['fullname'])){
		    $this->error['fullname'] = '用户名只能是字母/中文/数字/下划线';
		}
		
		if(!empty($this->model_account_customer->getFullnames($this->request->post['fullname']))){
		    $this->error['fullname'] = '该用户名已注册';
		}
