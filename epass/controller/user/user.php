...
class ControllerUserUser extends Controller {
	...
	//绑定商户	
		if (isset($this->request->post['user_merchant'])) {
		    $data['user_merchant'] = $this->request->post['user_merchant'];
		} elseif (!empty($user_info)) {
		    $data['user_merchant'] = $user_info['merchant_id'];
		} else {
		    $data['user_merchant'] = '';
		}
		
		if (isset($this->request->post['user_buyser'])) {
		    $data['user_buyser'] = $this->request->post['user_buyser'];
		} elseif (!empty($user_info)) {
		    $data['user_buyser'] = $user_info['buyser_id'];
		} else {
		    $data['user_buyser'] = null;
		}
		
		//var_dump( $data['user_merchant']);
		$data['merchants'] = $this->model_user_user->getMerchants();
        array_unshift($data['merchants'], null);
        
        $data['buyserid_list'] = $this->model_user_user->getBuysers();
		array_unshift($data['buyserid_list'], null);
        $data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
