...
function Install_InsertReport()
{
    ...
    typeid += gr_CodeBase;
    var obj = '<object id="_ReportOK" ' + typeid + ' width="0" height="0" VIEWASTEXT></object>'
    $("#divReport").html(obj);
...
function Install_Detect()
{
    ...
    {
    	layer.confirm('缺少插件是否按装？安装后请重启浏览器，并允许插件运行', {
		  btn: ['安装','取消'] //按钮
		}, function(){
			layer.close();
			GetReportPlugIn();
    ...
    {
        ConfirmTip(GetLanguageText("需要升级安装 锐浪报表插件 才能保证其正常运行打印面单，是否要下载升级安装？"), "GetReportPlugIn()", " HideWeightPop()", "ShowWeightPop() ");
        ...
        _ReportOK.Register(gr_UserName, gr_SerialNo);
        if ($(".mask").is(":hidden")) {
            ShowWeightPop();
        } 
    }
...
function GetReportPlugIn() {
    ...
    window.location.href =  gr_InstallPath + "/grbsctl5.exe";
