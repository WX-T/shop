...
class ModelDesignBanner extends Model {
	...
	public function editBanner($banner_id, $data) {
		...
		//是否有描述
		if(empty($data['describe_name'])){
			$data['describe_name'] = 1;
		}
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				if(empty($banner_image['width'])){
					$banner_image['width'] = 0;
				}

				if(empty($banner_image['height'])){
					$banner_image['height'] = 0;
				}

				//判断是否有宽高（魅力大牌图片设置宽高）
				if(empty($banner_image['img-width'])){
					$banner_image['img-width'] = 0;
				}

				if(empty($banner_image['img-height'])){
					$banner_image['img-height'] = 0;
				}
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			$banner_image_data[] = array(
				...
				//'describe' 				   => $banner_image_query->rows[0]['describe'],
			);
