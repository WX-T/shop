...
class ModelCatalogOption extends Model {
	...
	public function getOptionValueDescriptions($option_id) {
		...
		$option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value WHERE option_id = '" . (int)$option_id . "' ORDER BY sort_order ASC,option_value_id DESC");
	 ...
	 * @param string $opName
	 */
	public function getOptionInfo($opName = ''){
	    if(!$opName) return false;
	    $query = $this->db->query("SELECT o.option_id,o.type FROM `" . DB_PREFIX . "option` o,`" . DB_PREFIX . "option_description` od where o.option_id=od.option_id and od.name='".$opName."'");
	    return $query->row;
	}
	 ...
	 * @param string $opName
	 */
	public function getOptionValueInfo($option_id = '' , $value_name = ''){
	    $query = $this->db->query("SELECT ov.option_value_id FROM `" . DB_PREFIX . "option_value` ov,`" . DB_PREFIX . "option_value_description` ovd where ov.option_value_id=ovd.option_value_id and ov.option_id='".$option_id."' and ovd.name='".$value_name."'");
	     
	    return $query->row;
	}
	
	//添加选项值
	public function addOptionValue($option_id , $value_name){
	    $this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '', sort_order = '0'");
	    
	    $option_value_id = $this->db->getLastId();
	    
	    $this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '1', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape($value_name) . "'");
