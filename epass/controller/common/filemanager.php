...
class ControllerCommonFileManager extends Controller {
	...
	public function upload() {
		...
		if (!$json) {
			...
			$json['success'] = $this->language->get('text_uploaded');
 		    ...
 		    $result = $this->model_tool_qiniu->upload_image($directory . '/' . $filename);
 			if($result){
 			    $json['success'] = $this->language->get('text_uploaded');
 			}else{
 			    @unlink($directory . '/' . $filename);
 			    $json['error'] = $this->language->get('error_uploadqiniu');
 			}
		}
	...
	public function uploadProductImg () {
	    ...
	    // Make sure we have the correct directory
	    if (isset($mkdir)) {
	        $directory = rtrim(DIR_IMAGE . 'catalog/' . str_replace(array('../', '..\\', '..'), '', $mkdir), '/');
	    } else {
	        $directory = DIR_IMAGE . 'catalog';
	    }
	    $dir = 'catalog/'.$mkdir;
	    // Check its a directory没有目录择创建
	    if (!is_dir($directory)) {
	    ...
	    if (!$json) {
	        ...
	        $result = $this->model_tool_qiniu->upload_image($directory . '/' . $filename);
 			if($result){
 			    $json['code'] = '1';
 			    $json['success'] = $result;  //文件路径
 			    //保存到数据库
 			   //$this->load->model('catalog/product');
 			   /*  if($this->request->get['type']=='images'){
 			        $this->model_catalog_product->saveInsert($this->request->get['product_id'],$dir.'/'.$filename);
 			    }else{
 			        $this->model_catalog_product->saveProductImg($this->request->get['product_id'],$dir.'/'.$filename);
 			    } */
 			}else{
 			    @unlink($directory . '/' . $filename);
 			    $json['code'] = '0';
