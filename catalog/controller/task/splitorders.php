 ...
 *
 */
class ControllerTaskSplitOrders extends Controller{
    ...
    public function index() {
        ...
        if($orderList){
            ...
            foreach ($orderList as $order){
                ...
                $sqlproducts = "SELECT T1.order_product_id,T1.product_id,T1.quantity,T1.price,T1.total,T2.merchant_id,T2.line_id,T2.bonded FROM sl_order_product T1,sl_product T2 WHERE T1.`product_id`=T2.`product_id` AND T1.`order_id`='".$order['order_id']."' AND T1.refund='0'";
                $orderProducts = $this->db->query($sqlproducts)->rows;
                //单种单件商品超2000无法拆分,单个商品写入线路信息
                if(count($orderProducts) == 1 && $totalQuantity == 1){
                    //查询商品线路信息
                    $this->db->query("UPDATE " . DB_PREFIX . "order set is_split='1',bonded=".$orderProducts[0]['bonded'].",line_id=".$orderProducts[0]['line_id'].",merchant_id=".$orderProducts[0]['merchant_id']." WHERE order_id='".$order['order_id']."'");
                    continue;
                }
                ...
                if(count($orderProducts) == 1){
                    ...
                    for ($j = 0; $j < ceil($totalQuantity/$piceNum) ; $j ++){
                        $order['merchant_id'] = $orderProduct['merchant_id'];
                        $order['line_id'] = $orderProduct['line_id'];
                        $order['bonded'] = $orderProduct['bonded'];
                        $orderInfo = $order;
                ...
                //多商户多线路多种商品，按商户->线路->模式
                $productsArr = array();
                //拆分商户
                foreach ($orderProducts as $products){
                    $productsArr[$products['merchant_id']][] = $products;
                }
                //拆分商户下线路
                $productsArray = array();
                foreach ($productsArr as $key=>$prods){
                    foreach ($prods as $pro){
                        $productsArray[$key][$pro['line_id']][] = $pro;
                    }
                }
                //拆分商户线路下模式得到最终拆分数组
                $productList = array();
                foreach ($productsArray as $pr_key=>$merInfo){
                    foreach ($merInfo as $li_key=>$lineInfo){
                        foreach ($lineInfo as $bonded){
                            $productList[$pr_key][$li_key][$bonded['bonded']][] = $bonded;
                        }
                    }
                }
                foreach ($productList as $mer_id=>$merchant){
                    foreach ($merchant as $line_id=>$lineInfo){
                        foreach ($lineInfo as $bonded_id=>$protArr){
                            $order['merchant_id'] = $mer_id;
                            $order['line_id'] = $line_id;
                            $order['bonded'] = $bonded_id;
                           //单一来源产品种数和数量都为1
        ...
        foreach ($orderArr as $oid=>$norder){
            ...
            foreach ($norder as $neorder){
                ...
                foreach ($orderInfo as $key=>$value){
                    ...
                    }elseif($key == 'merchant_id'){
                        $value = $neorder['merchant_id'];
                    }elseif($key == 'line_id'){
                        $value = $neorder['line_id'];
                    }elseif($key == 'bonded'){
                        $value = $neorder['bonded'];
                    }
                    if($insertOrderSql == ''){
